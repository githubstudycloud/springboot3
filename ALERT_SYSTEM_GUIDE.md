...
### 🔄 锁等待增长监控
- **智能差值计算**: 系统不再依赖累计值，而是计算锁等待的增长差值
- **避免误报**: 防止因累计值过高导致的频繁报警
- **MySQL重启检测**: 自动检测并处理MySQL重启导致的累计值重置

### ⏰ 双重定时报告机制
- **常规健康报告**: 每日在0,5,7,9,12,14,16,18,20,22,23小时的第5分钟执行
- **锁等待专项报备**: 每30分钟检查一次锁等待增长情况
- **智能频率控制**: 根据系统负载和报警重要性调整报告频率

### 🎯 精准进程监控
- **有效SQL过滤**: 只推送info字段有实际SQL内容的进程
- **空值过滤**: 自动过滤NULL、空字符串和空白内容
- **性能优化**: 减少无效数据传输，提升监控效率

## 核心特性

### 🎯 三级分类
- **INFO**: 正常状态通知，连接恢复等
- **WARN**: 性能问题，警告级别事件，锁等待增长
- **ERROR**: 严重故障，数据库连接断开等
...
### 常规健康报告
执行时间：每日 0:05, 5:05, 7:05, 9:05, 12:05, 14:05, 16:05, 18:05, 20:05, 22:05, 23:05

包含内容：
1. Java应用程序状态检查
2. MySQL连接状态验证
3. 长时间运行SQL分析（仅info字段有值的进程）
4. 系统资源状态检查
5. 磁盘空间使用情况
6. 综合健康状态总结

### 锁等待专项报备
执行时间：每小时的第0分钟和第30分钟（如 10:00, 10:30, 11:00, 11:30...）
...
### 锁等待监控流程
1. **数据收集** → 获取当前锁等待累计值
2. **差值计算** → 与上次记录值比较，计算增长差值
3. **异常检测** → 识别MySQL重启等异常情况
4. **阈值判断** → 根据增长差值确定报警级别
5. **定期报备** → 每30分钟发送锁等待状况报告

### 进程过滤流程
1. **基础筛选** → 运行时间超过阈值且命令类型为Query
2. **内容验证** → 检查info字段是否有有效SQL内容
3. **空值过滤** → 排除NULL、空字符串和纯空白内容
4. **格式检查** → 确保info字段长度大于0且内容有意义
5. **结果推送** → 只推送通过所有验证的进程信息
...
注意：现在系统只会推送info字段有实际SQL内容的进程，空SQL和NULL值会被自动过滤。
